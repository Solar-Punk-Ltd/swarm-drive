...
describe("Swarm-CLI Integration Test: schedule", () => {
  ...
  it("runs an initial sync and then repeats every 1 second", async () => {
    ...
    const initResult = spawnSync(process.execPath, [CLI_PATH, "init", dataDir], {
      cwd: tmpDir,
    ...
    schedProc = spawn(process.execPath, [CLI_PATH, "schedule", "1"], {
      cwd: tmpDir,
      env: { ...process.env, BEE_SIGNER_KEY: signerKey },
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
      const feedOut = spawnSync(process.execPath, [CLI_PATH, "feed-ls"], {
        cwd: tmpDir,
        env: { ...process.env, BEE_SIGNER_KEY: signerKey },
        encoding: "utf-8",
      }).stdout.trim();
      ...
      if (parts.length >= 3) {
        ...
        if (/^[0-9a-fA-F]{64}$/.test(candidate)) {
          const lsResult = spawnSync(process.execPath, [CLI_PATH, "manifest-ls", candidate], {
            cwd: tmpDir,
            env: { ...process.env, BEE_SIGNER_KEY: signerKey },
            encoding: "utf-8",
          });
          if (lsResult.status === 0 && lsResult.stdout.includes("hello.txt")) {
    ...
    const finalLs = spawnSync(process.execPath, [CLI_PATH, "manifest-ls", foundManifest], {
      cwd: tmpDir,
      env: { ...process.env, BEE_SIGNER_KEY: signerKey },
      encoding: "utf-8",
    });
