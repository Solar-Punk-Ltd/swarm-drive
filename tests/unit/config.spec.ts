...
import { configGetCmd, configSetCmd } from "../../src/commands/config";
...
describe("config command", () => {
  ...
  describe("configSetCmd", () => {
    ...
    it("sets localDir", async () => {
      ...
      expect(saveConfigSpy).toHaveBeenCalledWith(expect.objectContaining({ localDir: "bar" }));
    ...
    it("sets watchIntervalSeconds to valid non-negative integer", async () => {
      ...
      expect(saveConfigSpy).toHaveBeenCalledWith(expect.objectContaining({ watchIntervalSeconds: 30 }));
    ...
    it("errors on invalid watchIntervalSeconds", async () => {
      ...
      expect(errSpy).toHaveBeenCalledWith(`Error: "-1" is not a valid non-negative integer for watchIntervalSeconds.`);
      exitSpy.mockRestore();
    });
    ...
    it("sets scheduleIntervalSeconds to valid non-negative integer", async () => {
      ...
      expect(saveConfigSpy).toHaveBeenCalledWith(expect.objectContaining({ scheduleIntervalSeconds: 60 }));
    ...
    it("errors on unknown key", async () => {
      ...
      expect(errSpy).toHaveBeenCalledWith(`Error: "fooBar" is not a valid configuration key.`);
      exitSpy.mockRestore();
    });
  });
